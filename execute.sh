...
do
  ...
  do
  	...
  	python main.py --$value --print --map="./maps/map$n.txt" | grep -e Time -e Node  -e Pont -e Left -e Acti | sed 's/^......//' > ./results/maps1/${value}_map$n.out
  	for ((i=0;i<10;i++))
  	do
  		echo "Seed = " $i
  		python main.py --$value --print --map="./maps/map$n.txt" | grep -e Time -e Node   -e Pont -e Left -e Acti | sed 's/^......//' >> ./results/maps1/${value}_map$n.out
...
#    python main.py --$value --print --map="./maps2/with_points/map$n.txt" | grep -e Time -e Node  -e Pont -e Left -e Acti | sed 's/^......//' > ./results/with_points/${value}_map$n.out
#    for ((i=0;i<10;i++))
#    do
#      echo "Seed = " $i
#      python main.py --$value --print --map="./maps2/with_points/map$n.txt" | grep -e Time -e Node   -e Pont -e Left -e Acti | sed 's/^......//' >> ./results/with_points/${value}_map$n.out
